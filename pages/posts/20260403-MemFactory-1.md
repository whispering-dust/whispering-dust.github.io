...
我训练时采用的是单卡 H20，按默认配置（1.7B 模型，2000条数据，epoch=2），并另外用一张卡部署qwen3-4B作为judge model。但是由于环境的问题，训练的过程极其缓慢。于是我尝试分析了一下训练速度慢的原因，并重新整理了一些加速的常见概念，单独整理在了 [训练加速探索笔记](./20260403-train_speedup.md) 里。
